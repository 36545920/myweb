  ...
  <div class="page">
    ...
    <div class="form">
      <div v-for="c in configs" :key="c.configKey" class="row">
        ...
        <input v-if="!isRegisterCount(c.configKey)" :value="c.configValue" @change="e => updateValue(c.configKey, (e.target as HTMLInputElement).value)" class="input" />
...
const labels: Record<string, string> = {
  total_user_quota: '用户空间总量 (字节)', total_pool_quota: '共享池总量 (字节)',
  default_user_quota: '新用户默认配额 (字节)', daily_register_limit: '每日注册上限',
  today_register_count: '今日注册数', register_date: '计数日期'
}
function configLabel(k: string) { return labels[k] || k }
function isRegisterCount(k: string) { return k === 'today_register_count' || k === 'register_date' }
...
.input { padding: 8px 12px; border: 1px solid #e8d5c0; border-radius: 6px; font-size: 13px; background: #fefaf2; outline: none; font-family: inherit; width: 200px; }
